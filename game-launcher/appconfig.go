...
2. Rename the copy to your language code, e.g. "de.json", "fr.json", "pt.json".
3. Translate ONLY the text on the right side of each line. Keep the keys (left) unchanged.
4. "__name" is the language name shown in the menu (e.g. "Deutsch").
...
Tip: you can also fix a built-in language (en/ru/es) — put e.g. "en.json" with ONLY the keys
...
2. Переименуйте копию в код языка, напр. "de.json", "fr.json", "pt.json".
3. Переводите ТОЛЬКО текст справа от каждой строки. Ключи (слева) не меняйте.
4. "__name" — название языка в меню (напр. "Deutsch").
...
Совет: можно поправить встроенный язык (en/ru/es) — положите, напр., "en.json" ТОЛЬКО с теми
